...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.linears = nn.ModuleList([self.SingleHeadAttention(embedding_dim, head_size) for i in range(num_heads)])
        # After the heads, add an output projection: 
        self.lin_layer = nn.Linear(attention_dim, attention_dim, bias=False)
        pass
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        #    tensor = head(embedded)
        #    multiHead = torch.cat((output, tensor), dim=2)
        tensors = [head(embedded) for head in self.linears]
        #tensors = self.linears(embedded)
        multiHead = torch.cat(tensors, dim=2)
        # Pass concatenated result through the output projection (W_O)
        tensor = self.lin_layer(multiHead)
